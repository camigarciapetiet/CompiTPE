	...
	public void handle(AnalizadorLexico analizadorLexico, BooleanHolder eot, IntHolder tipo_token, StringHolder token_actual, char ch) { //Solo sirve para nuestro automata
		...
		try {
			if (token_actual.valor == "&") { //El automata estaba en el estado 8, cuya unica salida es el simbolo &
				...
				token_actual.valor += "&";
				tipo_token.valor = 1;
				analizadorLexico.erroresLex.add("Warning en la linea "+ analizadorLexico.contadorLineas + ": '&' esperado despues de '&'");
			}
				...
				tipo_token.valor = 3;
				analizadorLexico.erroresLex.add("Warning en la linea "+ analizadorLexico.contadorLineas + ": double esperado luego de 'S'");
			}
			else if (((int) ch == 13) || ((int) ch == 10)) { //El automata estaba en el estado 11 y leyo un nl, lo reemplazamos por un espacio.
				ch = ' ';
				analizadorLexico.erroresLex.add("Warning en la linea "+ analizadorLexico.contadorLineas + ": ln inesperado dentro de una cadena de caracteres");
			}
		}
		catch(Exception e)
		{}
