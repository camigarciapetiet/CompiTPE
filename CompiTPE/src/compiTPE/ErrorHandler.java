...
public class ErrorHandler {
	public void handle(AnalizadorLexico analizadorLexico, BooleanHolder eot, IntHolder tipo_token, StringHolder token_actual, char ch) { //Solo sirve para nuestro automata
		...
		/*else if (token_actual.valor == "=") { //El automata estaba en el estado 6, cuya unica salida es el simbolo =
			...
			tipo_token.valor = 1;
			analizadorLexico.erroresLex.add("Error en la linea "+ analizadorLexico.contadorLineas + ": '=' esperado despues de '='");
		} Descontinuado al agregar TYPEDEF (tema particular 23) */
			...
			tipo_token.valor = 3;
			analizadorLexico.erroresLex.add("Error en la linea "+ analizadorLexico.contadorLineas + ": double esperado luego de 'S'");
		}
		else if (((int) ch == 13) || ((int) ch == 10)) { //El automata estaba en el estado 11 y leyo un nl, lo reemplazamos por un espacio.
			ch = ' ';
			analizadorLexico.erroresLex.add("Error en la linea "+ analizadorLexico.contadorLineas + ": ln inesperado dentro de una cadena de caracteres");
		}
	}
}
